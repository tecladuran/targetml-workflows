---
title: 'Tarragona Urine Measurements Preprocessing'
...
timestamp <- format(
  as.POSIXct(Sys.time(), tz = "Europe/Madrid"),
  "%Y-%m-%d_%H-%M"
)
...
peak_table_imputed <- as.data.frame(peak_table_imputed) %>%
  ...
  select(SampleID, class, matrix, patient_id, patient_condition, control_level, day, everything(), -FileName)
...
write.csv(peak_table$peak_table_duplicity, file.path(base_folder, "peak_table_duplicity.csv"), row.names = FALSE)
```
